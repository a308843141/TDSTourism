//
//  ValidationPhoneViewController.m
//  TDSTourism
//
...
@interface ValidationPhoneViewController ()
...
@implementation ValidationPhoneViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    if ([self.VpStr isEqualToString:@"1"]) {
        self.title = @"手机密码找回";
    }else{
        self.title = @"手机号注册";
    }
...
-(void)Createui{
    ...
    NSMutableAttributedString *hintString=[[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"我们向15041206187发送了一个验证码，请在下方输入%@",@""]];
    //获取要调整颜色的文字位置,调整颜色
    NSRange range1=[[hintString string]rangeOfString:@"15041206187"];
    ...
    self.EmailTextField.placeholder = @"请输入手机验证码";
